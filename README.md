...
One constraint governs the whole figure: the tops of both hands, both forearms
and his upper back all sit on the line `SUPPORT_Y` and together span
`SUPPORT_LEFT` to `SUPPORT_RIGHT` **with no break**. That line is scaled onto
the platform's top surface, so a gap anywhere along it is a place where objects
visibly hover with nothing beneath them. The script prints those metrics on
every run; they are mirrored in `ATLAS_ART` in `src/physics.js`, which also
derives `GROUND_Y` from them so his knee always meets the ground.
